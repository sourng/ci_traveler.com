...
$hotels_edit = NULL; // Initialize page object first

class chotels_edit extends chotels {
	...
	function LoadFormValues() {
		...
		if (!$this->h_feature_image->FldIsDetailKey) {
			$this->h_feature_image->setFormValue($objForm->GetValue("x_h_feature_image"));
		}
		...
		if (!$this->update_date->FldIsDetailKey) {
			...
			$this->update_date->CurrentValue = ew_UnFormatDateTime($this->update_date->CurrentValue, 0);
		}
	...
	function RestoreFormValues() {
		...
		$this->h_feature_image->CurrentValue = $this->h_feature_image->FormValue;
	...
	function RenderRow() {
		...
		$this->update_date->ViewValue = ew_FormatDateTime($this->update_date->ViewValue, 0);
		$this->update_date->ViewCustomAttributes = "";

		// h_online_status
		if (ew_ConvertToBool($this->h_online_status->CurrentValue)) {
			$this->h_online_status->ViewValue = $this->h_online_status->FldTagCaption(1) <> "" ? $this->h_online_status->FldTagCaption(1) : "Y";
		} else {
			$this->h_online_status->ViewValue = $this->h_online_status->FldTagCaption(2) <> "" ? $this->h_online_status->FldTagCaption(2) : "N";
		}
		$this->h_online_status->ViewCustomAttributes = "";

		// hotel_blocked
		if (ew_ConvertToBool($this->hotel_blocked->CurrentValue)) {
			$this->hotel_blocked->ViewValue = $this->hotel_blocked->FldTagCaption(1) <> "" ? $this->hotel_blocked->FldTagCaption(1) : "Y";
		} else {
			$this->hotel_blocked->ViewValue = $this->hotel_blocked->FldTagCaption(2) <> "" ? $this->hotel_blocked->FldTagCaption(2) : "N";
		}
		$this->hotel_blocked->ViewCustomAttributes = "";
			...
			$this->h_feature_image->EditValue = ew_HtmlEncode($this->h_feature_image->CurrentValue);
			...
			$this->update_date->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->update_date->CurrentValue, 8));
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// h_name
			$this->h_name->SetDbValueDef($rsnew, $this->h_name->CurrentValue, NULL, $this->h_name->ReadOnly);

			// h_slug
			$this->h_slug->SetDbValueDef($rsnew, $this->h_slug->CurrentValue, NULL, $this->h_slug->ReadOnly);

			// h_feature_image
			$this->h_feature_image->SetDbValueDef($rsnew, $this->h_feature_image->CurrentValue, NULL, $this->h_feature_image->ReadOnly);

			// h_description
			$this->h_description->SetDbValueDef($rsnew, $this->h_description->CurrentValue, NULL, $this->h_description->ReadOnly);

			// h_meta_key
			$this->h_meta_key->SetDbValueDef($rsnew, $this->h_meta_key->CurrentValue, NULL, $this->h_meta_key->ReadOnly);

			// h_deatail
			$this->h_deatail->SetDbValueDef($rsnew, $this->h_deatail->CurrentValue, NULL, $this->h_deatail->ReadOnly);

			// h_facilities
			$this->h_facilities->SetDbValueDef($rsnew, $this->h_facilities->CurrentValue, NULL, $this->h_facilities->ReadOnly);

			// h_address
			$this->h_address->SetDbValueDef($rsnew, $this->h_address->CurrentValue, NULL, $this->h_address->ReadOnly);

			// h_create
			$this->h_create->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->h_create->CurrentValue, 0), NULL, $this->h_create->ReadOnly);

			// dest_id
			$this->dest_id->SetDbValueDef($rsnew, $this->dest_id->CurrentValue, NULL, $this->dest_id->ReadOnly);

			// province
			$this->province->SetDbValueDef($rsnew, $this->province->CurrentValue, NULL, $this->province->ReadOnly);

			// whylike
			$this->whylike->SetDbValueDef($rsnew, $this->whylike->CurrentValue, NULL, $this->whylike->ReadOnly);

			// lang_spoken
			$this->lang_spoken->SetDbValueDef($rsnew, $this->lang_spoken->CurrentValue, NULL, $this->lang_spoken->ReadOnly);

			// map
			$this->map->SetDbValueDef($rsnew, $this->map->CurrentValue, NULL, $this->map->ReadOnly);

			// what_todo
			$this->what_todo->SetDbValueDef($rsnew, $this->what_todo->CurrentValue, NULL, $this->what_todo->ReadOnly);

			// h_id_cod
			$this->h_id_cod->SetDbValueDef($rsnew, $this->h_id_cod->CurrentValue, NULL, $this->h_id_cod->ReadOnly);

			// h_email
			$this->h_email->SetDbValueDef($rsnew, $this->h_email->CurrentValue, NULL, $this->h_email->ReadOnly);

			// h_contact_name
			$this->h_contact_name->SetDbValueDef($rsnew, $this->h_contact_name->CurrentValue, NULL, $this->h_contact_name->ReadOnly);

			// h_pass
			$this->h_pass->SetDbValueDef($rsnew, $this->h_pass->CurrentValue, NULL, $this->h_pass->ReadOnly);

			// h_contact_phone
			$this->h_contact_phone->SetDbValueDef($rsnew, $this->h_contact_phone->CurrentValue, NULL, $this->h_contact_phone->ReadOnly);

			// contact_fax
			$this->contact_fax->SetDbValueDef($rsnew, $this->contact_fax->CurrentValue, NULL, $this->contact_fax->ReadOnly);

			// star_rating
			$this->star_rating->SetDbValueDef($rsnew, $this->star_rating->CurrentValue, NULL, $this->star_rating->ReadOnly);

			// create_date
			$this->create_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->create_date->CurrentValue, 0), NULL, $this->create_date->ReadOnly);

			// update_date
			$this->update_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->update_date->CurrentValue, 0), NULL, $this->update_date->ReadOnly);

			// h_online_status
			$tmpBool = $this->h_online_status->CurrentValue;
			if ($tmpBool <> "Y" && $tmpBool <> "N")
				$tmpBool = (!empty($tmpBool)) ? "Y" : "N";
			$this->h_online_status->SetDbValueDef($rsnew, $tmpBool, NULL, $this->h_online_status->ReadOnly);

			// hotel_blocked
			$tmpBool = $this->hotel_blocked->CurrentValue;
			if ($tmpBool <> "Y" && $tmpBool <> "N")
				$tmpBool = (!empty($tmpBool)) ? "Y" : "N";
			$this->hotel_blocked->SetDbValueDef($rsnew, $tmpBool, NULL, $this->hotel_blocked->ReadOnly);
...
// Create page object
if (!isset($hotels_edit)) $hotels_edit = new chotels_edit();

// Page init
$hotels_edit->Page_Init();

// Page main
$hotels_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$hotels_edit->Page_Render();
?>
...
var CurrentForm = fhotelsedit = new ew_Form("fhotelsedit", "edit");

// Validate form
fhotelsedit.Validate = function() {
...
fhotelsedit.Form_CustomValidate = 
...
fhotelsedit.ValidateRequired = true;
<?php } else { ?>
fhotelsedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fhotelsedit.Lists["x_h_online_status[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fhotelsedit.Lists["x_h_online_status[]"].Options = <?php echo json_encode($hotels->h_online_status->Options()) ?>;
fhotelsedit.Lists["x_hotel_blocked[]"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fhotelsedit.Lists["x_hotel_blocked[]"].Options = <?php echo json_encode($hotels->hotel_blocked->Options()) ?>;
...
<?php } ?>
<?php $hotels_edit->ShowPageHeader(); ?>
<?php
$hotels_edit->ShowMessage();
?>
<?php if (!$hotels_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($hotels_edit->Pager)) $hotels_edit->Pager = new cPrevNextPager($hotels_edit->StartRec, $hotels_edit->DisplayRecs, $hotels_edit->TotalRecs) ?>
<?php if ($hotels_edit->Pager->RecordCount > 0 && $hotels_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($hotels_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $hotels_edit->PageUrl() ?>start=<?php echo $hotels_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($hotels_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $hotels_edit->PageUrl() ?>start=<?php echo $hotels_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $hotels_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($hotels_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $hotels_edit->PageUrl() ?>start=<?php echo $hotels_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($hotels_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $hotels_edit->PageUrl() ?>start=<?php echo $hotels_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $hotels_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fhotelsedit" id="fhotelsedit" class="<?php echo $hotels_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($hotels_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $hotels_edit->Token ?>">
<?php } ?>
...
<?php if ($hotels->hotel_id->Visible) { // hotel_id ?>
	<div id="r_hotel_id" class="form-group">
		<label id="elh_hotels_hotel_id" class="col-sm-2 control-label ewLabel"><?php echo $hotels->hotel_id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $hotels->hotel_id->CellAttributes() ?>>
<span id="el_hotels_hotel_id">
<span<?php echo $hotels->hotel_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $hotels->hotel_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="hotels" data-field="x_hotel_id" name="x_hotel_id" id="x_hotel_id" value="<?php echo ew_HtmlEncode($hotels->hotel_id->CurrentValue) ?>">
<?php echo $hotels->hotel_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<?php if ($hotels->h_feature_image->Visible) { // h_feature_image ?>
	<div id="r_h_feature_image" class="form-group">
		<label id="elh_hotels_h_feature_image" for="x_h_feature_image" class="col-sm-2 control-label ewLabel"><?php echo $hotels->h_feature_image->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $hotels->h_feature_image->CellAttributes() ?>>
<span id="el_hotels_h_feature_image">
<input type="text" data-table="hotels" data-field="x_h_feature_image" name="x_h_feature_image" id="x_h_feature_image" size="30" maxlength="250" placeholder="<?php echo ew_HtmlEncode($hotels->h_feature_image->getPlaceHolder()) ?>" value="<?php echo $hotels->h_feature_image->EditValue ?>"<?php echo $hotels->h_feature_image->EditAttributes() ?>>
</span>
...
<?php if ($hotels->h_create->Visible) { // h_create ?>
	<div id="r_h_create" class="form-group">
		<label id="elh_hotels_h_create" for="x_h_create" class="col-sm-2 control-label ewLabel"><?php echo $hotels->h_create->FldCaption() ?></label>
...
<?php if ($hotels->create_date->Visible) { // create_date ?>
	<div id="r_create_date" class="form-group">
		<label id="elh_hotels_create_date" for="x_create_date" class="col-sm-2 control-label ewLabel"><?php echo $hotels->create_date->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $hotels->create_date->CellAttributes() ?>>
<span id="el_hotels_create_date">
<input type="text" data-table="hotels" data-field="x_create_date" name="x_create_date" id="x_create_date" placeholder="<?php echo ew_HtmlEncode($hotels->create_date->getPlaceHolder()) ?>" value="<?php echo $hotels->create_date->EditValue ?>"<?php echo $hotels->create_date->EditAttributes() ?>>
</span>
...
<?php if ($hotels->update_date->Visible) { // update_date ?>
	<div id="r_update_date" class="form-group">
		<label id="elh_hotels_update_date" for="x_update_date" class="col-sm-2 control-label ewLabel"><?php echo $hotels->update_date->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $hotels->update_date->CellAttributes() ?>>
<span id="el_hotels_update_date">
<input type="text" data-table="hotels" data-field="x_update_date" name="x_update_date" id="x_update_date" placeholder="<?php echo ew_HtmlEncode($hotels->update_date->getPlaceHolder()) ?>" value="<?php echo $hotels->update_date->EditValue ?>"<?php echo $hotels->update_date->EditAttributes() ?>>
</span>
...
<?php
$selwrk = (ew_ConvertToBool($hotels->h_online_status->CurrentValue)) ? " checked" : "";
?>
<input type="checkbox" data-table="hotels" data-field="x_h_online_status" name="x_h_online_status[]" id="x_h_online_status[]" value="1"<?php echo $selwrk ?><?php echo $hotels->h_online_status->EditAttributes() ?>>
</span>
...
<?php
$selwrk = (ew_ConvertToBool($hotels->hotel_blocked->CurrentValue)) ? " checked" : "";
?>
<input type="checkbox" data-table="hotels" data-field="x_hotel_blocked" name="x_hotel_blocked[]" id="x_hotel_blocked[]" value="1"<?php echo $selwrk ?><?php echo $hotels->hotel_blocked->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $hotels_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php if (!isset($hotels_edit->Pager)) $hotels_edit->Pager = new cPrevNextPager($hotels_edit->StartRec, $hotels_edit->DisplayRecs, $hotels_edit->TotalRecs) ?>
<?php if ($hotels_edit->Pager->RecordCount > 0 && $hotels_edit->Pager->Visible) { ?>
