...
class cselling_rooms_add extends cselling_rooms {
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->day->FormValue)) {
			ew_AddMessage($gsFormError, $this->day->FldErrMsg());
		}
...
fselling_roomsadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($selling_rooms->day->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_month");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($selling_rooms->month->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_year");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($selling_rooms->year->FldErrMsg()) ?>");
...
<input type="text" data-table="selling_rooms" data-field="x_day" name="x_day" id="x_day" size="30" placeholder="<?php echo ew_HtmlEncode($selling_rooms->day->getPlaceHolder()) ?>" value="<?php echo $selling_rooms->day->EditValue ?>"<?php echo $selling_rooms->day->EditAttributes() ?>>
</span>
...
<input type="text" data-table="selling_rooms" data-field="x_month" name="x_month" id="x_month" size="30" placeholder="<?php echo ew_HtmlEncode($selling_rooms->month->getPlaceHolder()) ?>" value="<?php echo $selling_rooms->month->EditValue ?>"<?php echo $selling_rooms->month->EditAttributes() ?>>
</span>
...
<input type="text" data-table="selling_rooms" data-field="x_year" name="x_year" id="x_year" size="30" placeholder="<?php echo ew_HtmlEncode($selling_rooms->year->getPlaceHolder()) ?>" value="<?php echo $selling_rooms->year->EditValue ?>"<?php echo $selling_rooms->year->EditAttributes() ?>>
</span>
